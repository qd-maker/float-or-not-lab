...
一个面向小学高年级和初中生的浮力交互学习工具。它把「浮力为什么能托住物体」拆成概念实验、公式计算、真实题训练、AI 小老师、错题订正和针对性强化六个环节，让学生从看懂现象到能做题，再针对错误继续练习。
...
![针对性变式题练习](docs/images/float-lab-variant-practice.png)
...
- 根据原题、学生答案和动态错因生成同知识点变式题
- AI 输出必须经过 Pydantic 与题型业务规则校验
- 选择题答案必须对应唯一选项，填空题答案必须是数值
- 变式题由确定性代码判题，不让 AI 判断学生是否答对
- AI 不可用或输出不合格时自动使用本地变式题
- 强化结果计入本地学习统计，但不会污染原题错题本
...
| `POST /api/ai/generate-variant` | 根据错因生成并校验同类强化题 |
| `POST /api/practice/variant/submit` | 使用确定性规则判断强化题答案 |
...
6. 点击「再练一道同类题」，完成针对当前错因生成的强化题。
7. 查看学习统计中的强化次数、正确率和错题订正状态。
